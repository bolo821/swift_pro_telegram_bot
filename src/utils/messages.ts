...
Snipe, Trade and enhance your meme coin trading to the next level with lightning fast speed. Feel free to also utilize our mobile and desktop app.

For additional queries visit our <a href="https://docs.swiftbot.io/">Guide</>
`;
...
export async function getTokenStatusMessage(telegramId: string, chain: string, token: string) {
	...
	text += `📨 <code>t.me/Swift_Trading_Bot?start=${token}</code>`
	return {
		text,
		symbol
	};
...
export async function multiWalletViewMessage() {
	...
	response += `This is an overview page to see all your current wallets connect to the Swift Bot or Application.`;
	return response;
}
...
export function snipeMethodIdmessage(isBeta: boolean) {
	let response = `${isBeta ? "<b>🚨BETA FEAUTURE🚨</b>\nContract is not verified on chain, Swift bot have tried to decode the contract and display you possible solution\nSuccess is not guarantee\n" : ""}`
...
*\`https://t.me/Swift_Trading_Bot?start=swift_code_${code}\`*
`;
}
